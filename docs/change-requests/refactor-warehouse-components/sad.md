...
contain 39 after, with no expectation deleted.
...
Four files, 39 cases, none deleted.
...
`WarehouseRow.spec.tsx` 4, `WarehousePeopleList.spec.tsx` 3. Total 39.
...
| CR-RG-01           | §4.3's content-free move diff; then §4.6's case-count identity — 39 before, 39 after, distributed 25/7/4/3 — with every moved expectation's subject and expected value diff-identical. Any expectation edited to pass is an abort, not a finding                                                                                                                                                                                                                                                                           |
...
| CR-RG-05           | Build at `baseline_revision` and at `HEAD`; emit the Rollup output map, strip content hashes, rewrite moved source paths to their post-move form, sort, diff → empty. A baseline build that cannot be produced leaves this **unverified**                                                                                                                                                                                                                                                                                  |
