...
| CR-RG-01 | `no case is lost where two split targets share a title`                          | unit             | The baseline's **39** names all survive, plus the three additions review S3/S4/S5 required — **42**, distributed 27 / 7 / 4 / 4. The union alone cannot see one of a duplicated pair disappear.                                                                                                                                                       |
...
- **A case is silently dropped while a 1122-line spec becomes four files** → the case-inventory gate
  fails on both the union and the total of 42 (the baseline's 39 plus the three enumerated additions).
