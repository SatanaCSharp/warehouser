...
| CR-AC-04 | `each split component renders from plain props with no server stub`         | component | The four leaves and the two roots mount in isolation and render their own copy — proof the split introduced no hidden read.        |
...
| CR-RG-03 | `an actor without the revoke permission sees no withdraw control`                | component        | The row renders the email only; no control is emitted, and the gate's empty fragment leaves the surrounding markup unchanged.                         |
...
| CR-RG-05 | `the normalized module→chunk manifest is identical to baseline`                  | unit             | Built at both revisions, hashes stripped, moved paths rewritten, sorted → the diff is empty. No new eager chunk, every lazy route boundary preserved. |
...
| CR-RG-06 | `the manager-transfer concurrency case fails identically to baseline`            | integration      | Same assertion, same refusal-where-conflict-expected. Making it pass is out of scope and is itself a failure of this row.                             |
