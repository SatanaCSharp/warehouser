...
| schema_path                                                                         | origin                                                                                                                                                | confidence |
| ----------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- |
| `Error.details.violations[]` (`rule`, `path`, figures)                              | **derived** — sad.md §6.1 steps 5–6 require every violation collected and returned with its figures; the array shape is this stage's expression of it | medium     |
...
The two `medium` rows are declared incompleteness, not errors: the violation vocabulary and
`currentDisposition` are shapes no upstream artifact fixes, and `--reconcile` will tighten them if
`tasks` or `implement` settles a different one. No row is `low`: nothing in this contract was inferred
