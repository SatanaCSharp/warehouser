...
- Active Warehouse — The one Warehouse a Warehouse Member has selected to be shown, chosen from the Warehouses that member holds a membership in. It is presentation state that is retained for the member rather than for a device, persists until changed, and is set without the member acting only when they hold exactly one membership. NOT an input to any authorization decision, because every Warehouse-scoped request names the Warehouse it applies to. NOT Workspace, which is fixed for a User and never selected.
- Archived Warehouse — A Warehouse withdrawn from operation that no longer appears as selectable and accepts no Warehouse Capability, while Workspace Capabilities over it — renaming, restoring, membership assignment and revocation, and the protected Warehouse Manager transfer — remain available, its Roles, memberships, and records are retained, remain readable by its members, and it can be restored. NOT a deleted Warehouse, which this release does not provide. NOT a hidden Warehouse, since archiving withdraws a Warehouse from operation without withdrawing the account of what happened in it.
...
- A User belongs to a Workspace through a relation established when that User is created — at registration for a registrant, and from the Workspace owning the Warehouse they are created in for a member created by an authorized Warehouse Member — and never re-derived afterwards from that User's Warehouse memberships.
- Archiving a Warehouse withdraws its Warehouse Capabilities only; the Workspace Capabilities whose subject is that Warehouse record or a membership edge into it stay available while it is archived.
