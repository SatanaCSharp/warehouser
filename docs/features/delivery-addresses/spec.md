...
- [ ] Should `purchase-draft-line-ending.service.ts` keep a service's name and a service's directory now that it holds no service? Demoting `PurchaseDraftLineEndingService` to the module-level `assertAdmitsEnding` (2026-09-04 backend review, finding 4) left a file whose name and whose `domain/services/` placement both announce a collaborator that no longer exists, so `import { assertAdmitsEnding } from 'purchase-drafts/domain/services/purchase-draft-line-ending.service'` reads as reaching for a service — unlike `purchase-draft-assembly.service.ts`, where `assertApplied` sits beside a real `PurchaseDraftAssemblyService`. `server-architecture.md` §"Source structure" gives `domain/services/` to services, and §Services places a stateless helper at module level without saying which file holds it. The move is a rename plus two command imports and changes no behaviour. Deferred from the 2026-09-04 backend conformance re-review as advisory. Default now: leave it, and rename to `purchase-drafts/domain/purchase-draft-line-ending.assertions.ts` the next time either ending command changes. — owner: Backend Lead, due: before the next per-line ending change
- [ ] Should `customerHoldingName` and `remainingActiveDeliveryAddresses` stay `export`ed once their only consumers are in their own file? Moving both out of `customer.predicates.ts` (2026-09-04 backend review, finding 12) put them beside the assertion and the promotion that call them in `customer-address-book.service.ts`:70 and :83, which is the right home — but the `export` keyword now serves only `customer-address-book.service.spec.ts`, which is the shape finding 13 objected to, relocated rather than removed (`server-error-handling.md` §1, "Do not promote a predicate for hypothetical reuse"). Dropping `export` from `remainingActiveDeliveryAddresses` costs nothing because `nextMainDeliveryAddress` already covers it; `customerHoldingName`'s own spec block asserts case-sensitivity and the self-exclusion rule that `assertNameAvailable`'s cases do not restate, so removing its export means deciding whether that coverage moves or goes. Deferred from the 2026-09-04 backend conformance re-review as advisory. Default now: leave both exported, and narrow them when the file next changes. — owner: Backend Lead, due: before the next `customers` address-book change
- [ ] Should the per-line ending's guarded write carry a draft-state predicate? `arrival-confirmation.repository.ts`:208-219 predicates the line `UPDATE` only on `(id, purchaseDraftId, warehouseId, endingRecordedAt IS NULL)`; the draft-state condition lives solely in the pre-read `assertAdmitsEnding`, which is the check-then-write shape `purchase-draft-freeze.repository.ts`:26 forbids. A `closePurchaseDraft` with a reason committing between the pre-read and the write leaves the ending recorded and the Allocations applied against a draft that is Closed, and the subsequent closure `UPDATE` silently affects zero rows. Deferred from the 2026-09-04 independent review as stage-2; the window is narrow and no corruption is persisted. Default now: leave as is; add `state = 'ready_for_ordering'` to the `WHERE` when the ending path next changes. — owner: Backend Lead, due: before the next per-line ending change
- [ ] Should the freeze lock the draft and its linked Customer Orders before the agreement check? `ready-purchase-draft.command.ts`:86-135 reads the header and the disagreeing links without `FOR UPDATE`, and the snapshot capture is a separate statement after the guarded `UPDATE`. A redirect committing in between makes the capture read the new address, so a frozen line ships to one address while its captured demand says another — and because Address Drift compares the captured identifier against the order's current one (now equal), the violation reports no drift. `sad.md` §6.8 step 2 asked for the draft "under lock"; none is taken. Deferred from the 2026-09-04 independent review as stage-2. Default now: leave as is, and take the draft plus linked-order locks in ascending identifier order when the freeze path next changes. — owner: Backend Lead, due: before the next freeze change
- [ ] Should the Main-address transition and the customer-name insert be taken under lock, and should PostgreSQL `23505` map to a typed refusal? `set-main-customer-delivery-address.command.ts`:59-81 and `add-customer-delivery-address.command.ts`:86-95 read with the unlocked `listDeliveryAddresses` where `DeactivateCustomerDeliveryAddressCommand`:60 calls `lockDeliveryAddresses`, and `record-customer.command.ts`:64-97 is an unlocked check-then-insert. Under READ COMMITTED the loser of either race raises an unmapped `QueryFailedError` on `uq_customer_delivery_addresses_customer_main` or `uq_customers_warehouse_name`, so the member gets a 500 rather than the AC-03/AC-05 refusal — while `data-model.md` §Concurrency asserts "expected conflicts map to stable application errors". Follows the shipped `items.sku` precedent and is unprovable on the single-backend PGlite tier. Deferred from the 2026-09-04 independent review as stage-2. Default now: leave as is; route both address commands through `lockDeliveryAddresses` and map `23505` in one place when the `customers` write path next changes. — owner: Backend Lead, due: before the next `customers` write-path change
- [ ] Does `revise-purchase-draft-line.command.ts`:115-117's comment claim an isolation guarantee the transaction does not give? It says the address "cannot be deactivated between the proof and the write by a transaction that commits in between", but `findWarehouseDeliveryAddress` is a plain `findOne` with no `FOR UPDATE` under READ COMMITTED, so a concurrent deactivation committing after the read does let a line be pointed at an Inactive address. Impact is small — the freeze still captures the address as text — but the comment will stop a future reader from adding the guard. Deferred from the 2026-09-04 independent review as stage-2. Default now: correct the comment to record the accepted window rather than taking the lock. — owner: Backend Lead, due: before the next `revise-purchase-draft-line` change
- [ ] Are Delivery Address identifiers customer identity for the purposes of AC-09a? `purchase-draft.errors.ts`:151-166 carries `lineDeliveryAddressId` and `customerOrderDeliveryAddressId` to any actor holding `PURCHASE_DRAFTS:UPDATE`/`:READY` regardless of `CUSTOMERS:WATCH`, while `DemandSnapshotEntryRedacted` and `PurchaseDraftLineLinkRedacted` omit exactly those identifiers from the read path because they are treated as identity. A member without `CUSTOMERS:WATCH` can therefore correlate which linked orders share a destination — the same fact the read redacts. Fenced: only opaque UUIDs travel, no name, address text, access note or count, and `openapi.yaml`:1574-1577 sanctions the shape. Deferred from the 2026-09-04 independent review as stage-2, as a contract inconsistency rather than a live leak. Default now: leave as is, and settle it once — either withhold the two identifiers from the refusal or drop them from the redacted read's exclusion list. — owner: Security Lead, due: before the next `AC-09a` surface is added
- [ ] Should an Allocation aimed at a link of another line be refused in the domain rather than by a foreign key? `demand-allocation.service.ts`:126-137 never checks that `allocation.purchaseDraftLineLinkId` belongs to `line.purchaseDraftLineId`, so `applyAllocations` can write disagreeing triples. `fk_arrival_allocations_link` is composite and refuses them, so nothing wrong is persisted — but the outcome is an unmapped `QueryFailedError` and a 500 rather than the `allocation_out_of_bounds` 409 the contract declares. Deferred from the 2026-09-04 independent review as stage-2. Default now: leave the foreign key as the fence; add the violation rule beside `customer_order_not_unfulfilled` when the allocation path next changes. — owner: Backend Lead, due: before the next allocation change
- [ ] Should `listCustomerOrders` implement its declared `?customerId` and `?deliveryAddressId` filters, or should they be removed from the contract? `openapi.yaml`:878-893 declares both and :958 wires them to a 400 `CustomerFilterDenied` for an actor without `CUSTOMERS:WATCH`, but the accepted query is a `strictObject` of `itemId` and `state` only (`customer-orders-mutations.ts`:87-90), so `?customerId=…` is rejected as an unknown key with `request.invalid` and the `CustomerFilterDenied` response is unreachable. No acceptance criterion requires the filters — AC-15 requires the refusal of a disagreeing link, which the command does enforce. Deferred from the 2026-09-04 independent review as stage-2. Default now: leave as is, and decide at the next `customer-orders` read change whether to implement both filters with the observed-Permission refusal or delete them and record the removal in `api-sync-report.md`. — owner: Tech Lead, due: before the next `customer-orders` read change
- [ ] Should the identified form of `GET /customer-orders` be asserted over the wire? The only list test seeds an actor holding `CUSTOMER_ORDERS:WATCH` alone — the redacted branch — and asserts with `expect.objectContaining` (`customer-orders-http-contract.integration.spec.ts`:742-772), so a `customer`/`destination` leak on the list would not fail it. AC-24's own scenario, two rows side by side with one naming a Customer and one carrying a typed name, is proven only at the query tier over hand-built rows. The holder side is covered on the POST read-back, which takes a different filter and ordering path. Deferred from the 2026-09-04 independent review as stage-2. Default now: leave as is, and add the identified-form list case when the `customer-orders` read next changes. — owner: Backend Lead, due: before the next `customer-orders` read change
- [ ] Should AC-15a's revise-address and set-direct moments reach a database? Of the three moments AC-15a names, only the move to Ready for Ordering has an integration test (`ready-purchase-draft.command.integration.spec.ts`:509); the other two are proven only against repository doubles (`purchase-draft-line-delivery.spec.ts`:281). The composed PATCH route is covered for the AC-12 address proof but not for the disagreement refusal. Deferred from the 2026-09-04 independent review as stage-2, because the freeze moment — the one that produces a durable frozen violation — is the one covered. Default now: leave as is, and extend the PATCH describe when the direct-line agreement rule next changes. — owner: Backend Lead, due: before the next direct-line agreement change
- [ ] Should `hasExactlyOneMainActiveDeliveryAddress` stay in `customer.predicates.ts` when its only consumers are specs? Its callers are `delivery-address-book.spec.ts` and `customer.predicates.spec.ts` alone — the "predicate promoted for a consumer that is a test" shape `server-error-handling.md` §1 warns about, and the same shape already deferred above for `customerHoldingName` and `remainingActiveDeliveryAddresses` without covering this one. The assertions built on it are not tautological; the predicate has its own direct tests. Deferred from the 2026-09-04 independent review as stage-2. Default now: leave it, and move it into `src/test/` support beside the other address-book fixtures when the `customers` predicates next change. — owner: Backend Lead, due: before the next `customers` address-book change
- [ ] Can the two-backend ending race ever be proven in this repository? R2 (2026-09-04) took the `SELECT … FOR UPDATE` on the `purchase_drafts` row that `sad.md` §6.10 step 2 and `data-model.md`'s lock order prescribe, closing the defect where two concurrent last endings both evaluated the `NOT EXISTS` closure against a snapshot holding the other line un-ended, so neither closed the draft and it stayed in `ready_for_ordering` for good. The lock is asserted by statement shape — `arrival-confirmation.repository.integration.spec.ts` proves `FOR UPDATE` is the first statement, against the right row, before the line is resolved, and the assertion was mutation-checked to confirm it fails without the lock. What remains unproven is the serialization outcome itself: `jest.pglite.config.cjs` runs Postgres in single-user mode and records that "the repository deliberately holds no such specs", and `data-model.md` § Concurrency already concedes the tier cannot prove any race. The independent review suggested porting a two-`QueryRunner`/`pg_stat_activity` idiom from `purchase-draft-freeze.repository.integration.spec.ts`; that file only describes the technique in a comment and contains no such code, so there is nothing to port. Default now: the lock ships proven by shape, and the true race waits on a real-PostgreSQL test tier, which is its own piece of work. — owner: Backend Lead, due: with the real-PostgreSQL test tier
