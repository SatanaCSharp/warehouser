...
- [ ] Should `PurchaseDraftViewToggle` be nested under a `purchase-draft-workspace/components/` directory, with `PurchaseDraftWorkspace.tsx` moved beside it? `frontend-architecture.md` §"Components" and `placing-web-components.md` §"The nesting rule" put a sole-owned component one level under its owner, and `PurchaseDraftWorkspace` is the toggle's only importer — but the module `components/` root it joins already holds four single-consumer siblings that predate this feature (`PurchaseDraftList.tsx`, `PurchaseDraftDriftAlert.tsx`, `PurchaseDraftLineItemField.tsx`, `ValueAddingNoteField.tsx`), so conforming means moving files this change does not touch. Deferred from the 2026-09-04 frontend conformance review as advisory. Default now: leave the toggle flat and nest the whole group in one pass when the module is next restructured. — owner: Frontend Lead, due: before the next `purchase-draft` module change
