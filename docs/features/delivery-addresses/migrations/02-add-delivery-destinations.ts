...
export class AddDeliveryDestinations1786700100000 implements MigrationInterface {
  ...
  async up(queryRunner: QueryRunner): Promise<void> {
    ...
    // The line's own ending. `received_quantity` becomes `ending_quantity` because the column now
    ...
    await queryRunner.query(
      `ALTER TABLE purchase_draft_lines RENAME COLUMN received_quantity TO ending_quantity`,
    );
    await queryRunner.query(
  ...
  async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE purchase_draft_lines RENAME COLUMN ending_quantity TO received_quantity`,
    );
    await queryRunner.query(
