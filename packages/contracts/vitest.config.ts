...
const config = defineConfig({
  ...
  test: {
    ...
    coverage: {
      ...
      // attribute, and it reports the function as `skipped` rather than failed —
      // so the least-tested code is exactly what the gate would stay silent
      // about.
      //
...
/**
 * CRAP (`CC^2 * (1 - coverage)^3 + CC`) is opt-in, and `pnpm crap` is what opts
 * in: it is off for a plain `pnpm test`, which keeps the ordinary run free of
 * coverage instrumentation and free of a second way to fail.
 *
 ...
 *
 * **This gate is a placeholder here, not coverage.** In this package it scores
 * exactly one function — `graphemeCount`, CRAP 1.0 — against 318 passing tests.
 * The branchiest code in the package is invisible to it: the parser extracts
 * declared methods, and the validation logic lives in callbacks passed inline to
 * Zod's `.superRefine()` (`src/auth/index.ts:47`, the password validators at
 * `src/auth/index.ts:33` and `src/users/index.ts:11`), which it does not treat
 * as methods. That is structural — this package is schema declarations — so the
 * situation will not improve on its own, and a green `crap` here means nothing
 * was measured rather than that everything passed. It is kept so that anything
 * the parser *can* see starts being scored the day it is written, at the price
 * of one coverage-instrumented run.
 */
