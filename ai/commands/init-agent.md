...
Use the target agent's project-local locations, not user-global configuration. Skills are an
exception to the general adapter strategy: recursively copy every skill directory from its
canonical `ai/skills` source into the target's appropriate project-local skills directory. Never
install a skill or any file inside a skill as a symbolic link. For non-skill surfaces, prefer
relative symbolic links back to canonical files when the target supports links; otherwise generate
...
6. Install workspace and app scopes. Recursively copy each complete canonical skill directory,
   ...
   appropriate project-local skills directory. Do not create symbolic links for skill directories
   or their contents. Namespace app-specific skill collisions as `<app>-<skill>` at workspace
   scope. At app scope, the app-specific skill wins over a root skill with the same name. Never
