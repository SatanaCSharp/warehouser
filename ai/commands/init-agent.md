...
Also install the repository's hooks from `ai/hooks/` into the target's own hook surface. These are
not instructions the agent reads but work the runtime performs around it, so they hold for an agent
that never loaded the instruction: a guard that refuses a command before it runs, and index upkeep
that runs when a session starts and after the agent edits a file. `ai/hooks/README.md` states what
each hook does and how a target whose hook surface differs should call it;
`ai/hooks/claude-code.settings.json` is the draft settings block for Claude Code. A target lacking
one of those hook surfaces keeps the underlying rule through its instruction file — report that the
hook is uninstalled rather than substituting another mechanism.
...
8. Install the hooks from `ai/hooks/` into the target's hook configuration, merging into any
   existing hook settings rather than replacing them. Point the target at the canonical script —
   never inline a copy of a hook's rule into agent settings, which would leave two definitions to
   drift apart. For Claude Code, merge the whole `hooks` block of
   `ai/hooks/claude-code.settings.json` into `.claude/settings.local.json`; it carries the
   pre-command guard, the session-start index sync and the post-edit index sync together. For a
   target whose pre-command hook receives the command line as an argument or environment variable,
   call the guard's `--check "<command>"` mode and treat a non-zero exit as a refusal; a target
   with no session-start or post-edit surface keeps the index current by running `pnpm graph:sync`
   by hand. Verify the installed guard fires by running `ai/hooks/check-bypass-samples.sh`.
9. Remove stale entries only inside directories or managed blocks previously generated by this
...
10. Verify every installed non-skill link resolves, every generated non-skill copy matches its
