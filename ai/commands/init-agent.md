...
Also install the repository's pre-command guards from `ai/hooks/` into the target's own hook
surface. These are not instructions the agent reads but checks it runs before executing a command,
so they hold for an agent that never loaded the instruction. `ai/hooks/README.md` states what each
guard enforces and how a target that receives the command line differently should call it;
`ai/hooks/claude-code.settings.json` is the draft settings block for Claude Code. A target with no
pre-command hook surface keeps the underlying rule through its instruction file — report that the
guard is unenforced rather than substituting another mechanism.
...
8. Install the guards from `ai/hooks/` into the target's pre-command hook configuration, merging
   into any existing hook settings rather than replacing them. Point the target at the canonical
   script — never inline a copy of a guard's rule into agent settings, which would leave two
   definitions to drift apart. For Claude Code, merge the `hooks` block of
   `ai/hooks/claude-code.settings.json` into `.claude/settings.local.json`. For a target whose hook
   receives the command line as an argument or environment variable, call the guard's
   `--check "<command>"` mode and treat a non-zero exit as a refusal. Verify the installed guard
   fires by running `ai/hooks/check-bypass-samples.sh`.
