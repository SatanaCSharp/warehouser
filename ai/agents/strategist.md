...
- **B — Differentiation:** the wow-factor / strategic moat / unique angle. What makes this _worth_
...
- **Product-level only.** No concrete technology (datastore, broker, framework, library). Approaches
  differ in _strategy and scope_, not in tech stack — that's the `design` stage.
