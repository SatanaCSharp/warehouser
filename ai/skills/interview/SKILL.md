...
description: >
  ...
  architecture, refactor approach), but in an SDD repo the natural exit is /specify on
  the surviving idea. Triggers on "interview {slug}", "stress test {slug}", "challenge this",
  "poke holes", "rip this apart", "/interview {slug}", "погрилити", "розбери цю ідею",
  "розʼєби". Runs 3 phases (understand intent → surface tradeoffs and weak spots → propose
  new angles) through the available user-input mechanism, ends with a summary of risks, alternatives, and the next
...
scope. The boundary: this is an interview about the _idea_, not codebase archaeology. Ask the
...
One request through the available user-input mechanism, then commit (**default medium**). The dial is SDD-wide; interview's
...
1. **Every question uses the available user-input mechanism**, not free text — 2-4 concrete options, the
...
If the idea isn't stated in one sentence yet, ask for it in plain text without requiring a
...
Now actively propose through the available user-input mechanism: 2-3 alternative shapes (different audience, format,
...
## Final summary (plain text, not the structured user-input mechanism)
...
{one concrete verb — usually "/specify <slug>" once the idea survives}
```
...
(utility variant — `/clear` optional): _What I did_ (the revised idea + its weakest spot) + _Review_
(nothing on disk — the summary above is the artifact) + _Run next_: when the idea is a feature
you intend to build, `/specify <slug>` turns the survivor into a spec; otherwise resume
...
Once they answer, resume using the available user-input mechanism with a new angle.
...
- [`../_shared/ask-style.md`](../_shared/ask-style.md) — the structured user-input option-writing contract.
