...
description: >
  ...
  "tasks for {slug}", "plan the work for {slug}", "/tasks {slug}", "розбий на задачі {slug}",
  "декомпозиція {slug}", "список задач". Reads spec.md + sad.md + Accepted ADRs when present (+ data-model +
  ...
  tool-neutral. Hard-refuses if spec.md or sad.md is missing.
---
...
- For a declared web/mobile/desktop UI surface, `docs/features/<slug>/design-handoff.md` is required and must have `status: approved`, an exact `approved_frame`, and `approved_node_id`. Missing/unapproved → STOP and point to `design-ui <slug>`.

## Protocol

1. **Prereq check (hard).** Require spec.md + sad.md. Verify every ADR listed in `sad.md §9` exists and is Accepted; do not require an ADR when §9 says `None`. If `target_surfaces` contains a UI surface, also require an approved `design-handoff.md`.
