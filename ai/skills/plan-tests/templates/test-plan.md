...
| Component _(UI surface only)_      | A UI component exercised in isolation — props/state → rendered output + interactions.   | Render in a component harness; assert output + behaviour, no full app boot.               |
| Visual-regression _(web UI only)_  | The rendered UI diffed against an approved baseline image.                              | Snapshot the render; fail on an unintended visual diff; update the baseline deliberately. |
| E2E-through-UI _(UI surface only)_ | A user-story flow driven through the real UI, not just the API.                         | The flow exercised through the rendered UI against ephemeral dependencies.                |
