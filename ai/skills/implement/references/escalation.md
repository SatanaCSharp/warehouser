...
3. **Split the task.** If the task turns out to bundle two concerns (e.g. a validation rule _and_ an audit write), split it into two with a dep edge, and drive each with its own RED. Update `tasks.json` + `tracker.md` so the DAG stays the source of truth.
4. **Ask a human — the test may encode a wrong AC.** If the code is right and the _test_ asserts something the AC doesn't actually require (or the AC itself is wrong), STOP and ask. Surface: the failing line, the AC text, and why they conflict. **Never** edit the test to be less strict to make a wrong AC pass — fixing the AC is a `specify`/`clarify` change with the human in the loop.
