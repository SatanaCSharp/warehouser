# Settings — `.ai/sdd.local.md` (step 2)
...
1. If `.ai/sdd.local.md` is absent, write it with **the documented frontmatter below, followed by the «What each key does» section as the file's markdown body** — so the file is self-documenting: every key carries its default, its allowed values, and a plain explanation inline.
2. **Patch `.gitignore`** (create it if absent) to include `.ai/*.local.md` and `.worktrees/` — these are per-developer and must not be committed. (The `.ai/*.local.md` glob already covers `sdd.local.md`; don't add a redundant explicit line.)
3. Tell the user: «Wrote `.ai/sdd.local.md` with documented defaults — edit it to change how the pipeline behaves.»

## The documented frontmatter

<!-- This block is written verbatim to the top of `.ai/sdd.local.md`; the «What each key does»
...
team_mode: false # true → agent team via the runtime worker-delegation mechanism
...
cmd_lint: ''
cmd_vet: ''
model_test_author: balanced # fast | balanced | reasoning | inherit — adapter maps tiers to target models
model_implementer: balanced
model_reviewer: reasoning
judgment_model: reasoning # reasoning | premium — one switch for all judgment workers; per-role model_<role> wins
...
- **`judgment_model`** — `reasoning | premium` (default `reasoning`). One portable tier switch for all judgment workers — `reviewer` / `critic` / `devils-advocate` / `strategist` / `analyst`. The target-agent adapter maps the tier to an available model; a per-role `model_<role>` key still wins. Full precedence (highest wins): `environment > invocation > model_<role> > judgment_model > frontmatter > session`. Execution workers (`test-author` / `implementer`) and `explorer` / `researcher` are unaffected.
  - **Runtime mapping:** when these keys are set, map them to the target agent's supported model and reasoning controls. If no equivalent exists, use the closest available tier and report the fallback.
