...
The engine has three modes: **sequential single-agent TDD** (the floor everything degrades to), **agent team** (the runtime worker-delegation mechanism), and **dynamic Workflow**. The choice is deterministic — no judgement call at runtime.
...
- runtime: is the `Workflow` tool available? is `the runtime worker-delegation mechanism` available?
...
```
if team_mode AND parallel_eligible AND the runtime worker-delegation mechanism-available:
...
| `the runtime worker-delegation mechanism` not available at runtime | Skip the team branch; fall through to workflow or sequential.                                                                                                      |
