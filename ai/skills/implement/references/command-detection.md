...
1. **Settings override.** A non-empty `cmd_test_unit` / `cmd_test_integration` / `cmd_lint` / `cmd_vet` in `.ai/sdd.local.md` short-circuits everything. This is the escape hatch for unusual repos.
2. **Architecture-map frontmatter.** If `docs/system/architecture-map.md` exists and its frontmatter carries a non-empty `test_cmd` / `lint_cmd` (recorded by `survey` from what the repo actually uses), take it. `""` means unknown—fall through to the next step for that command.
...
5. **Language manifests** (the broad fallback — pick the toolchain the manifest implies):
   ...
   - any other manifest → there is no convention to trust: **ask the user for the commands** (and offer to save them to `.ai/sdd.local.md`) — never guess.
