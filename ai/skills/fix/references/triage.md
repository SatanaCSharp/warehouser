...
3. One clear winner → proceed. Several plausible owners (or zero hits but `docs/features/` is
   non-empty) → confirm with **one** the available user-input mechanism listing the candidates (phrasing per
...
- the handoff recommends `/survey` (map the codebase so future fixes have something to trace
