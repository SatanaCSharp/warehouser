...
- (Optional) `docs/features/<slug>/.size` — depth hint for _detail_ (XS/S may collapse a flow's internal steps), never for _coverage_. Absent → default to M **and say so loudly in the handoff** — «size M (default — no `.size`; run `/sdd:classify-size <slug>`)».
