...
8. **Lint + write + commit.** Suggest `spectral lint contracts/openapi.yaml` (add it to the project's check target if not yet wired). On a clean check, the files are written; propose commit `api: <slug> contract`. Then **emit the stage-handoff block** per [`../_shared/handoff.md`](../_shared/handoff.md) — _What I did_ + _Review_ (`contracts/openapi.yaml`, `api-sync-report.md`, + `events.md` if async) + _Run next_ (`/clear`, then `/sdd:tasks <slug>`).
