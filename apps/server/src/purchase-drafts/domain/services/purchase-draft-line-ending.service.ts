...
//
// It exists because there are exactly **two** callers. The kind of ending is a routing fact, so
...
// admits. A service with one caller would be a pass-through (server-architecture.md §Services);
// this one has two, which is what earns it.
//
...
@Injectable()
export class PurchaseDraftLineEndingService {
