...
export interface OnHandAdjustmentRuntime {
  readonly adjustmentId: () => string;
  readonly now: () => Date;
}

const defaultOnHandAdjustmentRuntime: OnHandAdjustmentRuntime = {
...
// AC-08/AC-09/AC-09a — the rules behind the one operation that writes an Item's On-hand Quantity
// (sad.md §6.3). The figure is a count the member made, never an increase or a decrease applied to
// the figure already stored (CONTEXT.md §Invariants), and it never moves without a reason, which is
// what keeps AC-18a true: nothing else in this release writes it, Arrival Confirmation included.
@Injectable()
export class OnHandAdjustmentService {
  constructor(
    ...
    @Optional()
    private readonly onHandAdjustmentRuntime: OnHandAdjustmentRuntime = defaultOnHandAdjustmentRuntime,
  ) {}
  ...
  async adjust(
    ...
    const adjustmentId = this.onHandAdjustmentRuntime.adjustmentId();
    const adjustedAt = this.onHandAdjustmentRuntime.now();
