// T6 — `items/domain/services/on-hand-adjustment.service.ts` does not exist yet. This is the
// legitimate RED for AC-09/AC-09a at the rule level, and for the non-enumerating Item resolution
// AC-03 fixes for every operation that names an Item. Proven against controlled repository doubles
// per server-architecture.md §Testing ("Test use cases with controlled repository doubles"), so no
// database is involved: the point of these cases is that **nothing is written at all**.
...
describe('OnHandAdjustmentService', () => {
  ...
  it.each(['', '   ', '\t\n'])(
    ...
    async (reason) => {
      ...
      const rejection = serviceWith(
        itemCatalogueRepository,
        itemStockAdjustmentRepository,
      ).adjust(currentUser, itemId, { countedQuantity: 12, reason });
  ...
  it('refuses an invalid figure without even resolving the Item', async () => {
    ...
    await expect(
      serviceWith(
        itemCatalogueRepository,
        itemStockAdjustmentRepository,
      ).adjust(currentUser, itemId, { countedQuantity: -1, reason: 'Counted' }),
  ...
  it.each([
    ...
    const rejection = serviceWith(
      itemCatalogueRepository,
      itemStockAdjustmentRepository,
    ).adjust(currentUser, itemId, { countedQuantity: 12, reason: 'Counted' });
  ...
  // repository's single cohesive operation, never two writes the service coordinates (sad.md §6.3,
  ...
  it('records the count, the reason, the acting member and the time in one repository call', async () => {
    ...
    const recorded = await serviceWith(
      itemCatalogueRepository,
      itemStockAdjustmentRepository,
    ).adjust(currentUser, itemId, {
      countedQuantity: 12,
  ...
  it('sets the figure to the count without reading the figure already stored', async () => {
    ...
    await serviceWith(
      itemCatalogueRepository,
      itemStockAdjustmentRepository,
    ).adjust(currentUser, itemId, { countedQuantity: 0, reason: 'Counted' });
