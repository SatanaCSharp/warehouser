...
// The Workspace-access half of
// `workspaces/.../workspace-http-contract.integration.spec.ts`, moved with its
// eleven handlers (CH-S2). It boots the real Nest module graph (`AppModule`),
// the real global Zod pipe and the real `GlobalHttpExceptionFilter`, then
// drives every `/api/v1/workspace/*` operation of `contracts/openapi.yaml`
// whose subject is Access — Workspace Roles, the Workspace Permission
// catalogue, Workspace Members, Users and Owner transfer — over actual HTTP.
// Only the wire is asserted: request-schema validation, the documented success
// shape, and the stable error code and status of each documented failure
// branch — including the non-enumerating cross-Workspace failure (AC-34) and
// the access-is-not-permitted denial (AC-30, AC-31). Every case is the one it
// made in its previous home; only the file changed (CR-RG-01), and these
