...
describeIntegration(
  'RegisterCommand workspace provisioning transaction',
  () => {
...
// DoD: "`access` receives only a `warehouseId` and a `userId`; a test asserts
// `access` imports nothing from `workspaces`." No repo-wide dependency-graph
// tool exists yet (T30 adds one); this mirrors the static-source-scan style
// already used by `workspaces/domain/module-boundaries.spec.ts` and
// `users/module-boundaries.spec.ts`, applied to the reverse direction those
// suites do not cover.
describe('access module boundaries', () => {
  const accessDirectory = join(__dirname, '../../../access');
  ...
  const accessSources = collectTsFiles(accessDirectory).map((filePath) => ({
    filePath,
    source: readFileSync(filePath, 'utf8'),
  }));

  it('discovers at least one source file under access/', () => {
    expect(accessSources.length).toBeGreaterThan(0);
  });

  it.each(accessSources)(
