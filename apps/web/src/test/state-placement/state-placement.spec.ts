...
//
// It scans the tree and the composed store rather than rendering, so it has no
// owner to sit beside: `frontend-architecture.md` § Testing colocates a slice
// test *with its owner*, and this guard's subject spans `auth`, `warehouse`,
// `workspace` and the root store, so no module owns it. That makes it a
// higher-level spec, which `docs/system/guides/placing-web-tests.md` files in
// its own directory under `src/test/` rather than one level above its subjects.
// `sad.md` §5.1 also fixes the workspace file manifest, which a colocated spec
// would enlarge.
